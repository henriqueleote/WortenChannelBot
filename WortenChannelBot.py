...
QUERY_URL = f'https://www.worten.pt/search?query=outlet%20{search_category}&sort_by=price&order_by=asc&facetFilters=seller_id%3A689dda97-efa4-4c6d-96bc-6f4bbfda8394&facetFilters=t_tags%3Ais_in_stock'
...
def getData(bot):
    ...
    if(p_element != None):
        ...
        if (int(product_count) > 0):
            ...
            product_name_span = product_list[0].findAll('span', {'class': 'produc-card__name__link'})
            product_link_anchor = product_list[0].findAll('a')
            product_price_euro = product_list[0].findAll('span', {'class': 'integer'})
            product_price_cent = product_list[0].findAll('span', {'class': 'decimal'})

            product_name = product_name_span[0].text
            product_link = PRODUCT_URL + product_link_anchor[0]['href']
            product_price = f'{product_price_euro[0].text},{product_price_cent[0].text}€'

            print(f'{product_name} -> {product_link} -> {product_price}')
        ...
        print("cookie problem, re-run the program until in works.")
